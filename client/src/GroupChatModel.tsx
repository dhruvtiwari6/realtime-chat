...
const GroupChatModel = ({
  ...
  const handleSearchUser = async (searchTerm :any) => {
    try {
      ...
      const filteredUsers : any = response.data.data.filter(
        (user : any) =>
  ...
  const handleRemoveUser = async (UserId :any) => {
    try {
      ...
      setSelectedChats((prevChats :any) => ({
        ...prevChats,
        users: prevChats.users.filter((user :any) => user._id !== UserId),
      }));
  ...
  const handleRenameGroup = async () => {
    ...
    try {
      ...
      setExistingChats((chat :any) => (chat._id === SelectedChats._id) ? {...chat ,name: groupName}: chat)
      handleFetchAgain();
      closeModal();
    } catch (error) {
  ...
  const handleSelectedUsers = async () => {
    ...
    const newUsers = selectedUsers.filter(
      (selectedUser :any) =>
        !SelectedChats.users.some(
          (existingUser :any) => existingUser._id === selectedUser._id
        )
    );
    ...
    try {
      ...
      const requests = newUsers.map((user :any) =>
        axios.put(
 ...
 // Assuming this function fetches the latest chat data
      setExistingChats((prevChats :any) =>
      ...
      setSelectedChats((prevChats:any) => ({...prevChats, users : [...prevChats.users, ...newUsers]}));
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content">
        <div>
          {SelectedChats.users.map((user :any) => (
            <span
              key={user._id}
              style={{
